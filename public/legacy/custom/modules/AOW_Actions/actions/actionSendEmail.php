...
include_once __DIR__ . '/../../../../modules/Emails/EmailsDataAddressCollector.php';
include_once __DIR__ . '/../../../../modules/Emails/EmailsControllerActionGetFromFields.php';
...
class actionSendEmail extends actionBase
{
    ...
    {
        return array('custom/modules/AOW_Actions/actions/actionSendEmail.js');
    }
    ...
    public function edit_display($line, SugarBean $bean = null, $params = array())
    {
        ...
        $html .= "<tr><td style='width: 10%'><label>From Email:</label></td>";
        $accs=$this->getFromFields($params['email_from']);
        $html .= "<td scope='row' style='width: 50%'><select name='aow_actions_param[".$line."][email_from]' id='aow_actions_param_email_from".$line."'>$accs</select></td></tr>";
        $html .= "<tr>";
        $html .= '<td id="name_label" scope="row" valign="top"><label>' . translate(
            "LBL_EMAIL",
            "AOW_Actions"
    ...
    public function run_action(SugarBean $bean, $params = array(), $in_save = false)
    {
        ...
        include_once __DIR__ . '/../../../../modules/EmailTemplates/EmailTemplate.php';
        ...
        if (isset($params['individual_email']) && $params['individual_email']) {
            foreach ($emails['to'] as $email_to) {
                ...
                if (!$this->sendEmail(array($email_to), $emailTemp->subject, $emailTemp->body_html, $emailTemp->body, $bean, $emails['cc'], $emails['bcc'], $attachments,$params['email_from'])) {
                    $ret = false;
            ...
            if (!$this->sendEmail($emails['to'], $emailTemp->subject, $email_body_html, $emailTemp->body, $bean, $emails['cc'], $emails['bcc'], $attachments,$params['email_from'])) {
                $ret = false;
                $this->lastEmailsFailed++;
            } else {
                $this->lastEmailsSuccess++;
    ...
    public function parse_template(SugarBean $bean, &$template, $object_override = array())
    {
        ...
        require_once __DIR__ . '/../../../../modules/AOW_Actions/actions/templateParser.php';
    ...
    public function sendEmail($emailTo, $emailSubject, $emailBody, $altemailBody, SugarBean $relatedBean = null, $emailCc = array(), $emailBcc = array(), $attachments = array(),$outbound_id=null)
    {
        ...
        if(!empty($outbound_id)){
            $outboundEmailAccount = BeanFactory::getBean('OutboundEmailAccounts',$outbound_id);
            $mail->From = $outboundEmailAccount->smtp_from_addr;
            $mail->FromName = $outboundEmailAccount->smtp_from_name;

            $emailObj->setOutboundBasedMailer($mail, $outboundEmailAccount);
        }
    ...
    private function getFromFields($from_selected)
    {
        global $current_user;
        global $sugar_config;
...
// Disable legacy behaviour permanently
...
//        }
        ...
        foreach ($results['data'] as $accs){
            if(empty($accs['attributes']['name'])&&empty($accs['attributes']['from'])){
                continue;
            }
            $selected='';
            if(!empty($from_selected)&&$from_selected==$accs['id']){
                $selected='selected';
            }
            $html.="<option value='{$accs['id']}' $selected>{$accs['attributes']['name']}  &lt;{$accs['attributes']['from']}&gt;</option>";
        }
        return $html;
    }
